...
首次连接运行以下命令；已有配置时复用。需要独立测试配置时才设置 `SELFGUIDE_BRIDGE_HOME`，收发命令使用同一环境变量。
...
在目标 Chrome 扩展管理页加载 skill 的 `extension/`，用 `bridge.py pairing` 返回的信息连接浏览器。配对码只填扩展，不写入聊天或公开报告。复用已有登录；不自动重启当前浏览器。同一浏览器仅连接一个活动桥接配置。

本地 SSH 转发服务器桥接端口到电脑相同端口，例如 `ssh -N -L 8765:127.0.0.1:8765 <服务器>`；两侧端口按实际配置填写。执行环境与桥接须保持运行，本工具不会自行唤醒 Codex。

配对一次即可；每个任务使用 `open --run` 自动创建独立 Chrome 窗口，共用原登录。旧的单标签页绑定仅供旧任务恢复。任务窗口不重复点配对，也不通过复制 profile 实现隔离。
