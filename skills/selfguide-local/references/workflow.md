...
`SELFGUIDE_LOCAL_HOME` 可指定独立连接目录，默认为 `~/.local/share/selfguide-local-bridge`；配对配置和任务队列不放 Git 仓库。一个服务对应一个配置的 selfguide 项目及一个本地标签页。

```bash
python <skill>/scripts/bridge.py init --project-url <selfguide项目完整URL>
python <skill>/scripts/bridge.py serve
# pairing 命令仅供用户在自己终端查看配对码；不要把输出写进仓库、网页或任务报告。
python <skill>/scripts/bridge.py pairing
python <skill>/scripts/session.py new --task-file <任务.txt> --workspace <工作区>/selfguide
...
URL 必须与浏览器实际地址一致。项目地址可能自动增加 `-selfguide`；`snapshot` 的错误提示或扩展绑定页可辅助核对，不修改项目 ID 来迁就错误线程。
