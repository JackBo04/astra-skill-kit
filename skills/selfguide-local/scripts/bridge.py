...
def validate(command, cfg):
    ...
    if action in {'compose', 'send', 'reply'}:
...
def main():
    ...
    for name in ACTIONS:
        ...
        if name != 'project': command.add_argument('--expect-url', required=True)
        if name in {'compose', 'send', 'reply', 'attach'}: command.add_argument('--file', type=Path, required=True)
    ...
    if args.action == 'init':
        ...
        if args.action != 'project': cmd['expected_url'] = args.expect_url
        if args.action in {'compose', 'send', 'reply'}: cmd['text'] = args.file.read_text()
