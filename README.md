...
**每个任务独立窗口，普通收发读取网页文字；程序等待回复，特殊异常才截图。**
...
| 操作 | 本分支做法 |
| --- | --- |
| 同时执行多个任务 | 按任务 ID 自动开独立窗口，分别排队与保存结果，共用登录 |
...
配对一次后，每个任务用 `session.py new` 建档，再 `bridge.py open --run <任务目录> --out <检查文件>` 自动创建独立窗口。该任务后续的 bridge 和 wait_reply 命令都带同一个 `--run`，无需切换当前窗口或重复配对。任务窗口自动平铺，避免完全遮挡造成网页停止刷新；原来的浏览器窗口不会被重新排布。

任务的窗口、附件、草稿、回复和等待状态分别保存；某一路生成、断开或关闭时，其他任务可以继续。窗口共用原浏览器登录与账户额度，没有复制账号资料。原来的单标签页绑定保留给旧任务恢复。
...
| 固定说明（字符数） | 精简前 | 现在 |
| --- | ---: | ---: |
| 普通任务首次读取，服务器版 | 8,497 | 2,486 |
| 普通任务首次读取，本地版 | 8,497 | 2,488 |
| 另需写作模块 | 628 | 243 |
| 每轮自动附加的交接提示 | 352 | 194 |

普通任务固定说明约减少 **71%**。对比基线为实验分支提交 `30e560b`，按文件文本字符计数；交接提示使用同一示例任务号。它不是 token 或账单测量，实际用量还取决于任务、正文长度与模型。
