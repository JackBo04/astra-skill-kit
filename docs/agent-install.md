...
默认安装到 `~/.agents/skills/chatgpt-supervised-<版本>`；若实际 Codex 环境需要其他发现目录，用 `--skills-dir <目录>` 指定。安装器会拒绝覆盖同名目录。已有安装时先确认来源和内容：相同版本直接复用；确需升级时备份旧 skill、保护用户定制，再执行安装。不要为了升级删除浏览器配置、会话记录或原 `chatgpt-supervised-work`。
...
```bash
python3 ~/.agents/skills/chatgpt-supervised-server/scripts/browserctl.py status
```
...
读取已有运行目录的 `run/astra-project.json` 取得项目名称和 URL；没有配置时，从用户指定或当前已登录页面确认 astra 项目，确实无法获取才请求 URL。不要输出含密钥的整个配置文件。

```bash
python3 ~/.agents/skills/chatgpt-supervised-server/scripts/browserctl.py setup --project-url '<实际astra项目URL>'
python3 ~/.agents/skills/chatgpt-supervised-server/scripts/browserctl.py start
python3 ~/.agents/skills/chatgpt-supervised-server/scripts/browserctl.py status
python3 ~/.agents/skills/chatgpt-supervised-server/scripts/browserctl.py url
```
...
本地版桥接没有第三方 Python 依赖。检查 `ASTRA_LOCAL_HOME` 或默认 `~/.local/share/astra-local-bridge` 的配置是否存在；已有项目和配对信息应复用，不重复 init。缺少项目 URL 时可读取原服务器浏览器的 astra 项目配置，只提取 URL，不迁移登录资料。

```bash
python3 ~/.agents/skills/chatgpt-supervised-local/scripts/bridge.py init --project-url '<实际astra项目URL>'
python3 ~/.agents/skills/chatgpt-supervised-local/scripts/bridge.py serve
```
...
连接准备好后，按照安装后的 skill 操作，不自行跳过发送状态记录。在 astra 建一条合成测试会话：
...
安装后若当前 Codex 会话还没发现新 skill，提示用户开启新会话。新任务使用 `$chatgpt-supervised-server` 或 `$chatgpt-supervised-local`，浏览器服务本身不会自动启动 Codex。
